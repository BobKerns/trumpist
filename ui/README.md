...
Now you can rename `src/App.css` to `src/App.scss` and update `src/App.ts` to import `src/App.scss`.
...
Import required reactstrap components within `src/App.ts` file or your custom component files:
...
To try it, delete all the code in `src/App.ts` and replace it with any of the examples on its website. The [Basic Example](https://reacttraining.com/react-router/web/example/basic) is a good place to get started.
...
1. Users aren't always familiar with offline-first web apps. It can be useful to
   ...
   starting point, you can make use of the logic included in [`src/serviceWorker.js`](src/serviceWorker.ts), which
